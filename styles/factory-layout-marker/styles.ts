...
export const CenterPanel = styled.section`
  ...
  box-shadow: var(--color-shadow, 0 1px 2px rgba(15, 23, 42, 0.04));
...
const ZONE_TONE_VARS: Record<ZoneTone, ToneVars> = {
  green: {
    ...
    background: 'var(--color-success-soft, rgba(5, 150, 105, 0.1))',
...
export const ZoneCard = styled.div<{
  ...
  border: 1px solid
    ${({ $selected, $dragOver }) => {
      ...
      return $selected ? 'var(--color-accent, #2563eb)' : 'var(--color-border, #e5e7eb)';
    }};
  border-radius: 22px;
  background: ${({ $selected, $dragOver }) => {
    if ($dragOver) {
      return 'var(--color-surface-hover, #f1f5f9)';
    }

    return $selected ? 'var(--color-accent-soft, rgba(37, 99, 235, 0.1))' : 'var(--color-surface-muted, #f8fafc)';
  }};
  color: var(--color-text-primary, #111827);
  cursor: grab;
  ...
  &:hover {
    border-color: ${({ $selected }) =>
      $selected ? 'var(--color-accent, #2563eb)' : 'var(--color-border-strong, #cbd5e1)'};
    background: ${({ $selected }) =>
      $selected ? 'var(--color-accent-soft, rgba(37, 99, 235, 0.1))' : 'var(--color-surface-hover, #f1f5f9)'};
  }
...
export const MarkerButton = styled.button<{
  ...
  box-shadow:
    ...
    var(--color-shadow, 0 1px 2px rgba(15, 23, 42, 0.04));
  color: var(--color-surface, #ffffff);
  outline: ${({ $selected }) =>
    $selected ? '5px solid var(--color-accent-soft, rgba(37, 99, 235, 0.1))' : '0 solid transparent'};
  outline-offset: 2px;
  touch-action: none;
  ...
  &:focus-visible {
    outline: 5px solid var(--color-focus, rgba(37, 99, 235, 0.22));
    outline-offset: 3px;
  }
`;
...
export const FloatingMarkerTooltip = styled.div`
  ...
  &::after {
    ...
    width: 0;
    height: 0;
    border-top: 7px solid var(--color-surface, #ffffff);
